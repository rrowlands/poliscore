...
import us.poliscore.model.IpGeolocationResponse;
...
//	}
	...
	public Optional<String> locateIp(String ip) {
		...
		val url = new URI("https://api.ipgeolocation.io/ipgeo?apiKey=" + secrets.getIpGeoLocationKey() + "&ip=" + ip).toURL();
		
		String json = IOUtils.toString(url.openStream(), "UTF-8");
		
		val resp = PoliscoreUtil.getObjectMapper().readValue(json, IpGeolocationResponse.class);
		
		Optional<String> result;
		
		if ("United States".equals(resp.getCountry_name())) {
			result = Optional.of(resp.getState_code().replace("US-", ""));
		} else {
			result = Optional.empty();
		}
